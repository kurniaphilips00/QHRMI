...
class Imt extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($keyword) {
            $ta = $this->rhModel->search($keyword);
        }
        else {
            $ta = $this->rhModel;
        }

        $result = $this->rhModel->paginate(10);
        $pager = $this->rhModel->pager;
        $data = ['rh' => $result, 'pager' => $pager, 'judul' => 'Riwayat Hidup Tenaga Ahli'];
    ...
    public function cetakCV($kode = null)
    {
        ...
        $text = 'Quantum';  //  Cetak tulisan Quantum
